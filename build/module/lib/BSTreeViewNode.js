...
import { EVENT_NODE_CHECKED, EVENT_NODE_COLLAPSED, EVENT_NODE_DISABLED, EVENT_NODE_EXPANDED, EVENT_NODE_RENDERED, EVENT_NODE_SELECTED, EVENT_NODE_UNCHECKED, EVENT_NODE_UNSELECTED, } from './BSTreeViewEventNames';
...
export default class BSTreeViewNode {
    ...
    _renderNode() {
        ...
        if (!this._domElement) {
            this._domElement = Template.node.cloneNode(true);
        ...
        for (let i = 0; this._domIndents.length < this._level - 1 && i < this._level - 1; i++) {
            this._domIndents.push(Template.indent.cloneNode(true));
        }
        ...
        if (this.hasChildren() || this.lazyLoad) {
            this._domIconExpand = Template.icon.expand.cloneNode(true);
            this._domElement.append(this._domIconExpand);
        }
        else {
            //Add an element for spacing
            this._domElement.append(Template.icon.empty.cloneNode(true));
        }
        ...
        // Add text
        this._domText = Template.text.cloneNode(true);
        ...
        if (this._options.showTags && this.tags) {
            ...
            this.tags.forEach((tag) => {
                const template = Template.badge.cloneNode(true);
     ...
     * @param options
     */
    toggleDisabled(options = new BSTreeViewDisableOptions()) {
     ...
     * @param options
     */
    toggleExpanded(options = new BSTreeViewMethodOptions()) {
     ...
     * @param options
     */
    setExpanded(state, options = new BSTreeViewMethodOptions()) {
     ...
     * @param options
     */
    _setVisible(state, options = new BSTreeViewMethodOptions()) {
        //Parse the passed options to an options object
        options = new BSTreeViewMethodOptions(options);
     ...
     * @param options
     */
    toggleSelected(options = new BSTreeViewSelectOptions()) {
     ...
     * @param options
     */
    setSelected(state, options = new BSTreeViewSelectOptions()) {
        //Parse the passed options to an options object
     ...
     * @param options
     */
    toggleChecked(options = new BSTreeViewMethodOptions()) {
        if (this._options.hierarchicalCheck) {
     ...
     * @param options
     */
    setChecked(state, options = new BSTreeViewMethodOptions()) {
        options = new BSTreeViewMethodOptions(options);
        // We never pass options when rendering, so the only time
        // we need to validate state is from user interaction
     ...
     * @param options
     */
    setDisabled(state, options = new BSTreeViewDisableOptions()) {
    ...
    _addCheckbox() {
        if (this._options.showCheckbox &&
            ...
            this._domCheckbox = Template.icon.check.cloneNode(true);
    ...
    _addIcon() {
        if (this._options.showIcon &&
            ...
            this._domIcon = Template.icon.node.cloneNode(true);
            this._domIcon.classList.add(...(this.icon || this._options.nodeIcon).split(' '));
    ...
    _addImage() {
        if (this._options.showImage && this.image) {
            this._domImage = Template.image.cloneNode(true);
     ...
     * @param options
     */
    _setSearchResult(state, options = new BSTreeViewMethodOptions()) {
        options = new BSTreeViewMethodOptions(options);
